...
#
# 覆盖：
#   1. demo-services / physics-services 单测（with_enabled 过滤/拒绝语义 + 原生服务实现）
#   2. main.rs 清单解析单测（三级配置/缺省全启/未知插件 id/空集）
#   3. tests/plugins_e2e.rs 组件级 E2E（子集命中/未启用如实报错/声明序确定性
#      + UV-035 双插件链：原生命中/穿透回落/链序正确性）
#   4. 真实二进制五场景：
#      A 缺省启动（无清单）→ health 显示 demo 7 服务 + physics 3 服务（双插件全启）
#      B 双插件子集清单    → health 仅显示各插件启用子集
#      C 双插件停用清单    → health 双 enabled=false
#      D 混合清单（demo 停用 + physics 子集）→ 插件间互不干扰
...
Write-Host "=== [1/5] demo-services / physics-services 单测 ===" -ForegroundColor Cyan
...
Write-Host "=== [3/5] 组件级端到端（tests/plugins_e2e.rs，含 UV-035 双插件链）===" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) { $failures++; Write-Host "[FAIL] evorule-server 构建失败" -ForegroundColor Red }
    ...
    # 四份清单（双插件子集/双插件停用/混合/非法未知名——sampling 为 demo 真实原生服务
    # 的近似拼写,实名是 sampling_service;physics 场景用 physics_energy 缺尾 e 的
    # physics_energ,故意触发 with_enabled 未知名 fail-fast）
    $subsetM = Join-Path $tmp "subset.json"
    $offM    = Join-Path $tmp "off.json"
    $mixM    = Join-Path $tmp "mix.json"
    $badM    = Join-Path $tmp "bad.json"
    '{ "plugins": { "demo-services": { "enabled": true, "services": ["config_persist"] }, "physics-services": { "enabled": true, "services": ["physics_energy"] } } }' |
        Set-Content -Path $subsetM -Encoding UTF8
    '{ "plugins": { "demo-services": { "enabled": false }, "physics-services": { "enabled": false } } }' |
        Set-Content -Path $offM -Encoding UTF8
    '{ "plugins": { "demo-services": { "enabled": false }, "physics-services": { "enabled": true, "services": ["physics_simulate"] } } }' |
        Set-Content -Path $mixM -Encoding UTF8
    '{ "plugins": { "demo-services": { "enabled": false }, "physics-services": { "enabled": true, "services": ["physics_energ"] } } }' |
    ...
    # 场景 A：缺省启动（无清单）= demo 全部 7 服务 + physics 全部 3 服务
    ...
    try {
        ...
        Assert-HealthPlugins $hB "场景B 双插件子集" "demo-services" $true @("config_persist")
        Assert-HealthPlugins $hB "场景B 双插件子集" "physics-services" $true @("physics_energy")
    ...
    try {
        ...
        Assert-HealthPlugins $hC "场景C 双插件停用" "demo-services" $false $null
        Assert-HealthPlugins $hC "场景C 双插件停用" "physics-services" $false $null
    } finally { if (-not $procC.HasExited) { Stop-Process -Id $procC.Id -Force } }

    # 场景 D：混合清单（demo 停用 + physics 子集）→ 插件间互不干扰
