...
> **本文档记录在实际集成 evorule-server 时踩过的所有坑**，供后续 LLM agent 和人类开发者参考。
>
...
**避坑要点**：这是 TCB 的**有意设计**——浮点数不确定性会破坏确定性执行保证。不要试图在 evorule 核心加 Float 变体（会违背 AGENTS.md 的 TCB 原语扩展谨慎原则）。业务层的浮点处理放在外部 I/O 服务中。
...
**避坑要点**：evorule 仓的 AGENTS.md 已记录此坑。CI 脚本和本地开发统一用 `pwsh`。
...
**避坑要点**：evorule 仓 AGENTS.md 已记录"复杂脚本写 `.ps1` 文件再用 `-File` 执行"。Python 同理——超过 3-4 行的命令一律写脚本文件。
