...
```bash
# 生成 JSON + 文本格式审计报告
mkdir -p audit-report-vX.Y.Z
cargo audit --json > audit-report-vX.Y.Z/cargo-audit.json
cargo audit > audit-report-vX.Y.Z/cargo-audit.txt
...
git tag -a vX.Y.Z -m "EvoRule Server vX.Y.Z
...
此命令会用默认严格模式运行 7 项检查（gate-bypass + 5 validate 脚本 + check_doc_safety）。`validate-release.ps1` 会检查 tag `vX.Y.Z` 存在且无更大 tag。

```bash
# 2. 确认 tag 在仓库存在
git tag -l vX.Y.Z                    # 本地
git ls-remote --tags origin vX.Y.Z   # Gitee
# GitHub: 暂缓（镜像仓未配置）
...
# GitHub: gh release delete vX.Y.Z --yes（暂缓）

# 2. 在仓库删除 tag
git tag -d vX.Y.Z                          # 本地
git push origin :refs/tags/vX.Y.Z          # Gitee
# git push github :refs/tags/vX.Y.Z        # GitHub（暂缓）

# 3. 在 README.md 标注"vX.Y.Z 已撤回，原因：XXX"
# 4. 修复后以 vX.Y.Z 重新发布（不覆盖已撤回的 vX.Y.Z）
```
