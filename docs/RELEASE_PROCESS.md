...
```bash
# 生成 JSON + 文本格式审计报告
mkdir -p audit-report-v0.5.0
cargo audit --json > audit-report-v0.5.0/cargo-audit.json
cargo audit > audit-report-v0.5.0/cargo-audit.txt
...
git tag -a v0.5.0 -m "EvoRule Server v0.5.0
...
此命令会用默认严格模式运行 7 项检查（gate-bypass + 5 validate 脚本 + check_doc_safety）。`validate-release.ps1` 会检查 tag `v0.5.0` 存在且无更大 tag。

```bash
# 2. 确认 tag 在仓库存在
git tag -l v0.5.0                    # 本地
git ls-remote --tags origin v0.5.0   # Gitee
# GitHub: 暂缓（镜像仓未配置）
...
# GitHub: gh release delete v0.5.0 --yes（暂缓）

# 2. 在仓库删除 tag
git tag -d v0.5.0                          # 本地
git push origin :refs/tags/v0.5.0          # Gitee
# git push github :refs/tags/v0.5.0        # GitHub（暂缓）

# 3. 在 README.md 标注"v0.5.0 已撤回，原因：XXX"
# 4. 修复后以 v0.5.0 重新发布（不覆盖已撤回的 v0.5.0）
```
