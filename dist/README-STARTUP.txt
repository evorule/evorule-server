...
4. 按页面内的「首跑向导」提示(约 3 分钟)完成登录与治理连接,
   即可开始使用;向导可随时跳过,以后可在「帮助」页重新运行
...
(evorule-rule,端口 18081,启动脚本已自动拉起)。

推荐方式:按浏览器内「首跑向导」第 2 步操作(自动预填地址,
明示体验包默认凭据,可一键填入)。手工方式:
...
- 浏览器内「首跑向导」第 3 步(推荐,加密保存到本机浏览器)
- 页面右上角「设置 → LLM 配置」手动填写
...
可查完整会话链),凭据保存在本机配置文件而非浏览器。

若安装时在安装器「配置 AI 助手」一步填写了 Key,插件已自动
配置并启用,跳过以下手工步骤。手工启用三步:
